...
plt.figure(figsize=(9, 5.8), layout='constrained')
...
data = dict(np.load('../data/s43588-025-00873-y_plotdata/plotdata_3obj_mps.npz'))
plt.fill_between(data['x'], np.min(data['y'], axis=0),
                 np.max(data['y'], axis=0), alpha=0.2)
plt.plot(data['x'], np.mean(data['y'], axis=0),
         label=fr'$p=6$ QAOA, MPS simulation ($\chi=50$) assuming 10,000 shots/s')

data = dict(np.load('../data/s43588-025-00873-y_plotdata/plotdata_3obj_ibm.npz'))
plt.fill_between(data['x'], np.min(data['y'], axis=0),
                 np.max(data['y'], axis=0), alpha=0.2)
plt.plot(data['x'], np.mean(data['y'], axis=0),
         label=fr'$p=6$ QAOA, ibm_fez assuming 10,000 shots/s')

data = dict(np.load('../data/s43588-025-00873-y_plotdata/plotdata_3obj_dcm.npz'))
plt.plot(data['x'], data['y'], linestyle=(0, (3, 2, 1, 2, 1, 2)), color=str(data['color']), label=data['label'])

data = dict(np.load('../data/s43588-025-00873-y_plotdata/plotdata_3obj_dpa-a.npz'))
...
plt.legend(loc=(0., -0.55), mode="expand", frameon=False)
plt.title('Three objective functions')

plt.savefig(f'3obj_results.pdf', bbox_inches='tight')
plt.show()
